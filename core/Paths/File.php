	...
	class File{
		...
		private function prepare(){
			...
			if( $this->Path()->is_relative() ){
				$this->absolute_path = Paths::get_root_path() . '/' . $this->original_path;
				$this->relative_path = trim( $this->original_path, '\\/' );
			} elseif( $this->Path()->is_absolute() ) {
				$this->absolute_path = $this->original_path;
				$this->relative_path = trim( str_replace( Paths::get_root_path(), '', $this->original_path ), '\\/' );
			}
		}
		...
		public function is_file(){
			if( $this->is_local() ) return is_file( $this->get_path() );
			return null;
		}
		...
		public function is_link(){
			if( $this->is_local() ) return is_link( $this->get_path() );
			return null;
		}
		...
		public function is_uploaded_file(){
			if( $this->is_local() ) return is_uploaded_file( $this->get_path() );
			return null;
		}
		...
		public function is_executable(){
			if( $this->is_local() ) return is_executable( $this->get_path() );
			return null;
		}
		...
		public function get_sub_files( $mask = [] ){
			...
			if( !array_key_exists( $maskKey, $this->cache_subFiles ) ){
				...
				if( $this->is_dir() ) foreach( scandir( $this->get_absolute_path() ) as $subFileName ){
					...
					$subFile = Paths::get( $subFilePath )->File();
					if( $subFile->is_dir() ){
						...
						if( is_array( $mask ) && count( $mask ) > 0 ){
							if( !in_array( Paths::file_extension( $subFileName ), $mask ) ) continue;
						}
		...
		public function get_next_file( $file_name ){
			return Paths::get( $this->dirname() . '/' . $file_name )->File();
		}
