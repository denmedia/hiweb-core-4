...
class PathsFactory {
     ...
     * @return Path
     */
    static function get($path_or_url_handle = ''): Path {
    ...
    static function get_by_id($attachment_id = 0): Path {
        if ( !array_key_exists($attachment_id, self::$cache_attachment_ids)) {
            if(function_exists('get_attached_file')) {
                ...
                //global $wpdb;
                //TODO
                self::$cache_attachment_ids[$attachment_id] = null;
            }
        }
     ...
     * @return Path_File
     */
    static function get_file($path_or_url_or_handle = ''): Path_File {
     ...
     * @return Path_Url
     */
    static function get_url($path_or_url_or_handle = ''): Path_Url {
     ...
     * @return string
     */
    static function get_root_path(): string {
        if ( !is_string(self::$root)) {
            self::$root_path = str_replace('\\', '/', $_SERVER['DOCUMENT_ROOT']);
            $patch = explode('/', trim(str_replace('\\', '/', __DIR__)));
            $patches = [];
            $last_path = '';
     ...
     * @version 1.0
     */
    static function get_root_url(): string {
     ...
     * @return string
     */
    static function get_current_url($trimSlashes = true): string {
     ...
     * @return string
     */
    static function get_size_formatted($size): string {
        $size = intval($size);
        if ($size < 1024) {
            return $size . ' ' . __('B');
        } elseif ($size < 1048576) {
     ...
     * @return string
     */
    static function get_freeFileName($filePath): string {
        if ( !file_exists($filePath)) {
            return $filePath;
        }
        $File = self::get($filePath)->file();
        for ($n = 1; $n < 9999; $n ++) {
            $test_path = $File->get_dirname() . '/' . $File->get_filename() . '-' . sprintf("%04d", $n) . '.' . $File->get_extension();
            if ( !file_exists($test_path)) {
                return $test_path;
            }
        }
        return $File->get_dirname() . '/' . $File->get_filename() . '-' . strings::rand(5) . '.' . $File->get_extension();
    }
    ...
    static function upload($fileOrUrl, $force_file_name = null) {
        if (is_array($fileOrUrl)) {
            ...
            $fileName = trim($force_file_name) == '' ? self::get($fileOrUrl)->file()->get_basename() : $force_file_name;
            $tmp_name = $fileOrUrl;
     ...
     */
    static function get_attachment_id_from_url($url): int {
        $attachment_id = 0;
        $dir = wp_upload_dir();
     ...
     * @return Path
     */
    static function get_bySearch(array $pathsArray): Path {
