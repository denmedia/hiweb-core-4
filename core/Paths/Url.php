	...
	class Url{
		...
		private function prepare(){
			if( !is_array( $this->prepare_data ) ){
				$this->prepare_data = [];
				if( strpos( $this->url, Paths::get_root_path() ) === 0 ) $this->url = str_replace( Paths::get_root_path(), '', $this->url );
		...
		public function is_dirs_intersect( $haystackUrl ){
			...
			return ( count( $this->get_dirs_intersect( $haystackUrl ) ) >= Paths::get( $haystackUrl )->Url()->dirs()->count() );
		}
