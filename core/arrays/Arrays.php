		...
		public function each( $callable ){
			...
			if( is_callable( $callable ) ){
				foreach( $this->get() as $key => $value ){
					$R[ $key ] = call_user_func_array( $callable, [ $value, $key ] );
				}
			}
			return $R;
		}
...
//			return $this->array[ $key ];
		...
		public function get_value_first( $default = null ){
			return $this->get_value_by_index( 0 );
		}
			...
			if( !is_array( $key ) || count( $key ) == 1 ){
				...
				foreach( $key as $subkey ){
					if( $this->has_key( $subkey ) ){
						return ( new Arrays( $this->array[ $subkey ] ) )->value_by_key( array_slice( $key, 1 ), $default );
					} else break;
				}
			}
			return $default;
			...
			if( !is_array( $index ) || count( $index ) == 1 ){
				...
				foreach( $index as $ind ){
					...
					if( $this->has_index( $ind ) ){
						return ( new Arrays( array_values( $this->array )[ (int)$ind ] ) )->value_by_index( array_slice( $index, 1 ), $default );
					} else break;
				}
			}
			return $default;
		...
		public function set_value( $key = null, $value = null ){
			if( is_null( $key ) ) $this->array[] = $value; else $this->array[ $key ] = $value;
		}
		...
		public function key_by_value( $search_value, $use_regexp = false, $default = null ){
			foreach( $this->array as $key => $val ){
				if( !is_array( $val ) && !is_object( $val ) ){
					...
					$sub_find = ( new Arrays( $val ) )->key_by_value( $search_value, $use_regexp );
					if( !is_null( $sub_find ) ){
		...
		public function unset_key( $key ){
			...
			if( !is_array( $key ) || count( $key ) == 1 ){
				...
				$new_array[ $first_key ] = ( new Arrays( $new_array[ $first_key ] ) )->unset_key( $key );
			}
