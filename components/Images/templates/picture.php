...
$pictureAttributes = get_array($pictureAttributes);
foreach ($pictureAttributes as $key => $val) {
    if (is_null($val)) $pictureAttributes->unset_key($key);
}
?>
<picture <?= $pictureAttributes->get_as_tag_attributes() ?>>
    <?php if (($tryWebP || \hiweb\components\Images\ImagesFactory::$useWebPExtension) && \hiweb\components\Client\Client::get_instance()->is_support_WebP()) {
        if ($dimension->width < 1000 && $dimension->height < 1000 && !get_client()->is_mobile()) {
            $big_size = $this->sizes()->get([ $dimension->width * 1.75, $dimension->height * 1.75, 1 ]);
