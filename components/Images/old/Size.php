	...
	class Size extends Path{
		...
		public function width(){
			if( intval( $this->width ) < 1 ){
				$this->width = intval( $this->Image()->width() );
			}
			return $this->width;
		}
		...
		public function height(){
			if( intval( $this->height ) < 1 ){
				$this->height = intval( $this->Image()->height() );
			}
			return $this->height;
		}
		...
		public function get_dimension_size_string(){
			return $this->width() . 'x' . $this->height() . ( $this->is_crop() ? 'c' : '' ) . '-' . $this->File()->extension();
		}
