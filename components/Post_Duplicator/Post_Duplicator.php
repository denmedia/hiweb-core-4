...
class Post_Duplicator {
            ...
            add_action('current_screen', function() {
                ...
                if (get_current_screen()->base == 'edit' && get_current_screen()->post_type != '' && isset($_GET['hiweb-post-duplicate'])) {
                    if (wp_verify_nonce($_GET['_wpnonce'], 'hiweb-post-duplicate')) {
                        ...
                        add_admin_notice(__('Failed to create a duplicate entry because the validation key does not match', 'hiweb-core-4'))->options()->error();
                    }
                }
            });
        }
    ...
    static function _post_row_actions($actions, $post) {
        if ($post instanceof WP_Post) {
            ...
            $actions['hiweb-theme-post-duplicator'] = '<a data-hiweb-duplicate="' . $post->ID . '" href="' . get_url()->set_params([ 'hiweb-post-duplicate' => $post->ID, '_wpnonce' => wp_create_nonce('hiweb-post-duplicate') ])->get() . '">' . __('Duplicate', 'hiweb-core-4') . ' <b>' . $wp_post_type->labels->singular_name . '</b></a>';
        }
    ...
    static function do_duplicate($post_id, $force_parent_post_id = null, $_main_proccess = true) {
        ...
        if (is_int($destination_post_id)) {
            ...
            ///
            if ($_main_proccess) add_admin_notice(__('A duplicate entry has been created.', 'hiweb-core-4') . ' <a href="' . get_edit_post_link($destination_post_id) . '">' . $post_type->labels->edit_item . ' "' . htmlentities($destination_post->post_title) . '"</a>')->options()->success();
        } else {
            if ($_main_proccess) add_admin_notice(__('Failed to create duplicate entry', 'hiweb-core-4'))->options()->error();
        }
        ///
