jQuery(document).ready(function(){
    ...
    var hiweb_field_image = {
        ...
        init: function (root) {
            jQuery(root).on('click', '[data-click-wp-media]', function (e) {
                e.preventDefault();
                hiweb_field_image.event_click_select(root);
            }).on('click', '[data-click-clear]', function (e) {
                e.preventDefault();
                hiweb_field_image.deselect_file(root);
            });
        },
        ...
        event_click_select: function (root) {
            ...
            gallery_window.on('select', function () {
                hiweb_field_image.select_file(root, gallery_window.state().get('selection').first().toJSON());
            });
            gallery_window.open();
        },
        ...
        edit_file: function (current) {
            ...
            gallery_window.on('select', function () {
                hiweb_field_image.select_file(current, gallery_window.state().get('selection').first().toJSON());
            });
            gallery_window.open();
        },
    ...
    jQuery('.hiweb-type-field-image').each(function () {
        hiweb_field_image.init(this);
    });

    jQuery('body').on('hiweb-field-repeat-added-row', '.hiweb-type-field-image input[name]', function () {
        hiweb_field_image.init(jQuery(this).closest('.hiweb-type-field-image'));
    });
});