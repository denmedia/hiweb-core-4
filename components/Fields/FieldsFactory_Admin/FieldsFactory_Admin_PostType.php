...
class FieldsFactory_Admin_PostType {
	 ...
	 * @version 1.1
	 */
	static private function get_current_query($append_post_type_query = [], $force_wp_post = null) {
		...
		if ( is_array( $append_post_type_query ) )
			$query['post_type'] = array_merge( $query['post_type'], $append_post_type_query );
		return $query;
	}
	...
	static function _save_post($post_ID, $post, $update) {
		if ( !$update || !array_key_exists( 'hiweb-core-field-form-nonce', $_POST ) || !wp_verify_nonce( $_POST['hiweb-core-field-form-nonce'], 'hiweb-core-field-form-save' ) )
			return;
