<?php
	...
	class FieldsAdminFactory{
		...
		static function get_current_values( $location_query = null ){
			//$current_fields = FieldsFactory::
			$R = [];
			$fields = FieldsFactory::get_field_by_location( is_array( $location_query ) ? $location_query : self::get_current_fields_query() );
			foreach( $fields as $Field ){
				if( array_key_exists( 'post_type', $location_query ) && metadata_exists('post', $location_query['post_type']['ID'], 'meta_key_to_check') ){
		...
		static function get_ajax_form_html( $query ){
			...
			IncludesFactory::Css( __DIR__ . '/FieldsAdmin.css' );
			ob_start();
			if( count( FieldsFactory::get_field_by_location( $query ) ) == 0 ) return;
			?>
			<div class="hiweb-components-form-ajax-wrap form-preload" data-fields-query="<?= htmlentities( json_encode( $query ) ) ?>" data-scripts-done="<?= htmlentities( json_encode( wp_scripts()->done ) ) ?>">
		...
		static function get_ajax_form_hock(){
			...
			$debug = 0;
			$values = self::get_current_values( $query );
		...
		static function get_form_html( $fields_array, $field_values = null ){
			...
			foreach( $fields_array as $Field ){
				...
				if( is_array( $field_values ) ){
					if( array_key_exists( $Field->ID(), $field_values ) ) $value = $field_values[ $Field->ID() ];
				}
				...
				$fields_html[] = str_replace( '<!--input-->', $Field->get_admin_html( self::$the_form_field_value, self::$the_form_field_name ), ob_get_clean() );
			}
			///
			...
			foreach( FieldsFactory::get_field_by_location( $query ) as $Field ){
				...
				if( $Field->get_allow_save_field( array_key_exists( $field_name, $_POST ) ? $_POST[ $field_name ] : null ) ){
					if( array_key_exists( $field_name, $_POST ) ){
						update_post_meta( $post_ID, $Field->ID(), $Field->get_sanitize_admin_value( $_POST[ $field_name ] ) );
					} else {
						update_post_meta( $post_ID, $Field->ID(), $Field->get_sanitize_admin_value( null ) );
					}
				}
			}
		}
